...
\\$\forall G\in M_n(\mathbb{R}_+^*)$ tel que $\textbf{1}^TG=\textbf{1}^T,\exists!X\in C,\forall x\in C,\lim\limits_{N\to\infty}G^Nx=X$.
...
\\$\exists \alpha_1,\dots,\alpha_n\in\mathbb{C},x=\alpha_1X+\sum\limits_{k=2}^n \alpha_kv_k$. $(*)$
\\Soit $k\in[\![2,n]\!]$.On cherche à calculer $\textbf{1}^Tv_k$.
\\$Gv_k=\lambda_kv_k$
\\$\textbf{1}^T(Gv_k)=\textbf{1}^T(\lambda_kv_k)$
\\$(\textbf{1}^TG)v_k=\lambda_k\textbf{1}^Tv_k$
\\$\textbf{1}^Tv_k=\lambda_k\textbf{1}^Tv_k$
\\$(1-\lambda_k)\textbf{1}^Tv_k=0$
\\Comme $1-\lambda_k\neq0$, donc $\textbf{1}^Tv_k=0$. $(**)$
\\On cherche à calculer $\alpha_1$.
\\$\textbf{1}^Tx=\textbf{1}^T\left(\alpha_1X+\sum\limits_{k=2}^n \alpha_kv_k\right)$
\\$1=\alpha_1\textbf{1}^TX+\sum\limits_{k=2}^n\alpha_k\textbf{1}^Tv_k$
\\$1=\alpha_1\times1+\sum\limits_{k=2}^n\alpha_k\times0$ d'après $(**)$.
\\D'où $\alpha_1=1$.
\\$(*)$ devient $x=X+\sum\limits_{k=2}^n \alpha_kv_k$.
\\On applique $G$ $N$ fois.
\\$G^Nx=G^N\left(X+\sum\limits_{k=2}^n \alpha_kv_k\right)$
\\$G^Nx=G^NX+\left(\sum\limits_{k=2}^n \alpha_kG^Nv_k\right)$
\\$G^Nx=1^NX+\left(\sum\limits_{k=2}^n \alpha_k\lambda_k^Nv_k\right)$
\\Or, d'après (1) et (3) du théorème de Perron-Frobenius, $\forall k\in[\![2,n]\!],|\lambda_k|<1$.
\\On passe à la limite.
\\$\lim\limits_{N\to\infty}G^Nx=X$.
\\On rappelle que X est unique, nous l'avons démontré à la fin du (2) du théorème de Perron-Frobenius.
\\Conclusion : le point fixe ne dépend pas du vecteur d'état de départ $x$. Il ne dépend que de la matrice $G$.
\end{document}